...
function ProjectCard({ project, delay = 0 }: { project: (typeof projects)[number]; delay?: number }) {
  return (
    <Reveal delay={delay}>
      <article className="card-surface card-lift group h-full rounded-3xl border border-primary/25 p-6">
...
export function MoreProjects() {
  return (
    <section id="work" className="scroll-mt-28 py-14 lg:py-20">
      <Reveal>
        <p className="font-mono text-[11px] uppercase tracking-[0.3em] text-flame">03 — Everything else</p>
        <h2 className="mt-4 font-display text-[clamp(2rem,6vw,4rem)] font-extrabold uppercase leading-[0.9] tracking-tight">The rest of<br />the build log</h2>
      </Reveal>
      ...
      <Reveal delay={120}>
        <div className="mt-8 grid gap-6 rounded-3xl border border-primary/25 bg-ink-soft p-7 sm:p-10 lg:grid-cols-[auto_1fr] lg:items-center lg:gap-12">
