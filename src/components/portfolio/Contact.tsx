...
export function Contact() {
  ...
  return (
    <footer id="contact" className="scroll-mt-32 -mx-5 border-t border-border bg-ink-soft/40 px-5 py-14 sm:-mx-8 sm:px-8 lg:py-20">
      <div>
        ...
        <Reveal delay={140}>
          <div className="mt-7 flex flex-wrap gap-3">
            <a href="mailto:christopherzarraga31@gmail.com" className="rounded-lg border border-border px-6 py-3 font-mono text-xs uppercase tracking-[0.15em] text-volt transition-colors duration-180 hover:bg-primary hover:text-primary-foreground">christopherzarraga31@gmail.com</a>
      ...
      {showResume && <div id="inline-resume" className="mt-10 flex flex-col overflow-hidden rounded-lg border border-border card-surface"><div className="flex items-center justify-between border-b border-border px-5 py-3"><span className="font-mono text-[11px] uppercase tracking-[0.25em] text-muted-foreground">Christopher Zarraga — Resume</span><Button type="button" onClick={() => setShowResume(false)} aria-label="Close resume" variant="outline" size="sm" className="rounded-lg border-border bg-transparent font-mono text-[11px] uppercase tracking-[0.15em] text-foreground shadow-none hover:border-flame hover:bg-transparent hover:text-flame">Close</Button></div><img src={resumeImage} alt="Christopher Zarraga Jimenez resume" className="block h-auto w-full bg-background object-contain" /></div>}
    </footer>
  );
}
