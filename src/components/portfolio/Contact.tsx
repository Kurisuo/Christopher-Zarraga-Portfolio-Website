...
export function Contact() {
  ...
  return (
    <footer id="contact" className="scroll-mt-32 -mx-5 border-t border-border bg-ink-soft/40 px-5 py-14 sm:-mx-8 sm:px-8 lg:py-20">
      ...
      {showResume && <div id="inline-resume" className="mt-10 flex flex-col overflow-hidden rounded-xl border border-primary/25 card-surface shadow-2xl"><div className="flex items-center justify-between border-b border-primary/25 px-5 py-3"><span className="font-mono text-[11px] uppercase tracking-[0.25em] text-muted-foreground">Christopher Zarraga — Resume</span><Button type="button" onClick={() => setShowResume(false)} aria-label="Close resume" variant="outline" size="sm" className="rounded-full border-border bg-transparent font-mono text-[11px] uppercase tracking-[0.15em] text-foreground shadow-none hover:border-flame hover:bg-transparent hover:text-flame">Close</Button></div><img src={resumeImage} alt="Christopher Zarraga Jimenez resume" className="block h-auto w-full bg-background object-contain" /></div>}
    </footer>
  );
}
